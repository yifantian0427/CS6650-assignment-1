...
public class Deduplicator {
    ...
    public boolean shouldProcess(String roomId, String messageId) {
        if (roomId == null || roomId.isBlank() || messageId == null || messageId.isBlank()) return true;
    ...
    private static final class RoomWindow {
        ...
        private RoomWindow(int cap) { this.cap = cap; }
        private synchronized boolean addIfAbsent(String id) {
            if (set.contains(id)) return false;
            set.add(id);
            order.addLast(id);
            while (order.size() > cap) set.remove(order.removeFirst());
            return true;
        }
    }
}
